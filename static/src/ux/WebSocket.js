...
Ext.define ('Ext.ux.WebSocket', {
    ...
    safeSend: function (data) {
        ...
        if (me.isReady ()) {
            if(data.length>me.maxFrameSize)
                me.longDataSend(data)
            else
                me.ws.send (data);
        }
    ...
    longDataSend: function(data) {
        var i = 0, n = 0, k = 0, s = '', me = this       
            ,token = Math.random() + ''
            ,l = data.length;
        while(i<l){
            s = token.length + ':' + token + n + ':';
            n++;
            k = this.maxFrameSize - s.length - 6;
            str = 'frame' + ((i+k)>=l? 'e':'p') + s
            me.ws.send (str + data.substr(i, k));
            i += k;
        }
    },
	 ...
	 * @private
	 */
	sendEventMessage: function (events, data, opid) {
    ...
    jsonEncodeClr: function(obj) {
        if(obj.data) {
            var out = {}
            for(var i in obj.data) {
                if(Ext.isArray(obj.data[i]) || Ext.isString(obj.data[i]) || Ext.isNumber(obj.data[i]))
                    out[i] = obj.data[i]  
